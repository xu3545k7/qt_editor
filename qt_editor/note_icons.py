...
def _load(rel: str) -> QPixmap:
    if rel not in _pix_cache:
        _pix_cache[rel] = QPixmap(os.path.join(_GRAPHIC, rel))
    return _pix_cache[rel]


def _frame(kind: str, hand: int, width: int) -> QPixmap:
    folder, prefix = _ORIG[kind]
    lr = 'l' if hand == 1 else 'r'
    return _load(os.path.join('orig', f'{folder}_{lr}',
                              f'{prefix}_{lr}_{max(1, min(10, width)):02d}.png'))
...
def note_type_pixmap(note_type: int, hand: int = 0) -> QPixmap:
    ...
    if nt == 2:                                          # long：身體往上長
        ...
        img = _frame('white', hand, 3)
        ratio = img.height() / float(img.width()) if not img.isNull() and img.width() else 0.43
        arrow = _load('LeftStacArrow-v2.png' if hand == 1 else 'RightStacArrow-v2.png')
